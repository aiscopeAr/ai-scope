...
export default async function AuthorPage({
  ...
  // Fetch articles authored by this author (tagged __author:<slug>)
  let articles: {
    id: string;
    slug: string;
    titleAr: string;
  ...
  try {
    ...
    // DB unavailable — render profile without articles
  }

  const accentRgb = author.accentColor; // hex like #6366f1
    ...
    <div className="min-h-screen" dir="rtl">
          ...
          <div className="flex flex-col items-center gap-8 sm:flex-row sm:items-start">
            ...
            <div className="relative shrink-0">
              <div
                ...
                style={{ outline: `4px solid ${author.accentColor}`, outlineOffset: "3px" }}
              >
                <Image
              ...
              {/* AI badge */}
              <span
                className="absolute -bottom-1 -left-1 rounded-full border border-white/10 bg-[#0d0d12] px-2 py-0.5 text-[10px] font-bold"
      ...
      <section className="container mx-auto max-w-4xl px-4 py-12">
        <h2 className="mb-6 text-xl font-black text-white">
          مقالات {author.nameAr}
          {totalCount > 0 && (
            <span className="mr-2 text-base font-normal text-slate-500">({totalCount})</span>
          )}
        </h2>

        {articles.length === 0 ? (
          <p className="text-slate-500">لا توجد مقالات بعد.</p>
        ) : (
            ...
            {articles.map((article) => {
              ...
              return (
                <Link
                  ...
                  className="group flex flex-col overflow-hidden rounded-xl border border-white/6 bg-white/2 transition hover:border-violet-500/30 hover:bg-white/4"
                >
                  <div className="relative h-44 w-full shrink-0 bg-white/4">
                    {article.imageUrl ? (
                      ...
                      <div
                        ...
                        style={{
                          background: `linear-gradient(135deg, ${author.accentColor}20, #1e1b4b40)`,
                        }}
                      >
                    ...
                    <div className="absolute bottom-2 right-2">
                      <span className="rounded-full bg-black/60 px-2 py-0.5 text-[11px] font-medium text-slate-300 backdrop-blur-sm">
                        {article.category.nameAr}
                      </span>
                    </div>
                  </div>
                  <div className="flex flex-1 flex-col p-4">
                    <h3 className="mb-2 line-clamp-2 flex-1 text-sm font-bold leading-snug text-slate-200 transition-colors group-hover:text-violet-300">
                      {article.titleAr}
                    </h3>
                    {article.excerpt && (
                    ...
                    {timeAgo && (
                      <p className="text-[11px] text-slate-600">{timeAgo}</p>
                    )}
                  </div>
                </Link>
              );
