      ...
      <main className="min-h-screen" dir="rtl">
        ...
        >
          ...
          <div className="pointer-events-none absolute inset-0 overflow-hidden">
            <div className="absolute -top-24 -right-24 h-72 w-72 rounded-full opacity-[0.07]"
              style={{ background: "radial-gradient(circle, #6366f1, transparent)" }} />
            <div className="absolute -bottom-16 -left-16 h-60 w-60 rounded-full opacity-[0.05]"
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {topCats.map((cat) => (
                  <Link
                    ...
                    className="flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-sm font-medium transition hover:scale-[1.03]"
        ...
        <div className="container mx-auto px-4 py-10 max-w-5xl">
          ...
          {/* ── Editor Picks — Product Hunt list style ──────────────────── */}
          ...
          <div className="mb-12 grid gap-6 md:grid-cols-2">
            ...
            {trending.length > 0 && (
              <section>
                ...
                <div className="overflow-hidden rounded-[12px] border" style={{ borderColor: "var(--border-subtle)" }}>
                  {trending.map((t, i) => (
                    <div key={t.id} className={i < trending.length - 1 ? "border-b" : ""}
                      style={{ borderColor: "var(--border-subtle)" }}>
                      <Link
                        href={`/ai-tools/${t.slug}`}
                        className="group flex items-center gap-3 p-3.5 transition hover:opacity-80"
            ...
            {newest.length > 0 && (
              <section>
                ...
                <div className="overflow-hidden rounded-[12px] border" style={{ borderColor: "var(--border-subtle)" }}>
                  {newest.map((t, i) => (
                    <div key={t.id} className={i < newest.length - 1 ? "border-b" : ""}
                      style={{ borderColor: "var(--border-subtle)" }}>
                      <Link
                        href={`/ai-tools/${t.slug}`}
                        className="group flex items-center gap-3 p-3.5 transition hover:opacity-80"
              ...
              {TOOL_CATEGORIES.filter(c => c.value !== "other").map((cat) => {
                ...
                return (
                  <Link
                    ...
                    className="flex flex-col items-center justify-center gap-2 rounded-[10px] border p-4 text-center transition hover:scale-[1.02]"
          ...
          {/* ── All tools — full searchable directory ───────────────────── */}
          <section>
            <div className="mb-4 flex items-center gap-2">
              <span className="text-lg">🔍</span>
              <h2 className="text-base font-bold" style={{ color: "var(--text-primary)", fontFamily: "var(--font-serif)" }}>
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              {[
                ...
                <Link
                  key={item.href}
                  href={item.href}
                  className="flex items-center gap-2 rounded-[8px] border px-3 py-2.5 text-sm font-medium transition hover:opacity-80"
                  style={{ borderColor: "var(--border-medium)", color: "var(--text-secondary)", backgroundColor: "var(--bg-surface)" }}
                >
                  <span>{item.icon}</span>
